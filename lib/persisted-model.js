...
module.exports = function(registry) {
   ...
   */
  function uploadInChunks(largeArray, processFunction, cb) {
    var self = this;
    var chunkArrays = [];
  ...
  function tryReplicate(sourceModel, targetModel, since, options, callback) {
    ...
    function getSourceChanges(cb) {
      downloadInChunks(options.filter, function(filter, pagingCallback) {
        sourceModel.changes(since.source, filter, options.tenant, pagingCallback);
    ...
    function getDiffFromTarget(sourceChanges, cb) {
      uploadInChunks(sourceChanges, function(smallArray, chunkCallback) {
    ...
    function createSourceUpdates(_diff, cb) {
      ...
      if (diff && diff.deltas && diff.deltas.length) {
        ...
        uploadInChunks(diff.deltas, function(smallArray, chunkCallback) {
    ...
    function bulkUpdate(_updates, cb) {
      ...
      updates = _updates;
      uploadInChunks(updates, function(smallArray, chunkCallback) {
  ...
  PersistedModel.createUpdates = function(deltas, cb) {
    ...
    deltas.forEach(function(change) {
      ...
      switch (type) {
        case Change.CREATE:
        case Change.UPDATE:
          changeLookupByModelId[change.modelId] = update;
          break;
        case Change.DELETE:
          updates.push(update);
          break;
    ...
    var keys = Object.keys(changeLookupByModelId);

    var idName = Model.dataSource.idName(Model.modelName);
    var where = {};
    where[idName] = {inq: keys};
    Model.find({where: where}, function(err, instances) {
      if (err) return cb(err);
      var err;
      instances.forEach(function (inst) {
        var update = changeLookupByModelId[inst[idName]];
        delete changeLookupByModelId[inst[idName]];
      ...
      var missingData = Object.keys(changeLookupByModelId);
      if (missingData.length) {
        err = new Error(g.f('Missing data for changes: %s', missingData.join(', ')));
        return cb && cb(err);
      }
  ...
  PersistedModel.enableChangeTracking = function() {
    ...
    if (runtime.isServer && cleanupInterval !== -1) {
      // initial cleanup
      cleanup();
  ...
  function rectifyOnSave(ctx, next) {
    ...
    // If we have a single instance then perform an optimized rectify 
    if (instance) {
      ctx.Model.rectifyInstChange(instance, ctx, reportErrorAndNext);
    } else if (ids) {
  ...
  PersistedModel.rectifyInstChange = function(instance, ctx, callback) {
    var Change = this.getChangeModel();
    Change.rectifyModelChange(this.modelName, instance, ctx, callback);
  };
